...
locals {
  ...
  userDefinedString_7 = substr(var.userDefinedString, 0, 7 - length(postfix_3))
  vm-name = regex("[\\/\"'\[\]:|<>+=;,?\*@&]","${local.env_4}${local.serverType_3}-${local.userDefinedString_7}${local.postfix_3}")
